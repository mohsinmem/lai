...
const scoringWorker = require('./netlify/functions/scoring-worker.js');
const crypto = require('crypto');

async function verify() {
  console.log('--- Phase 1: Gate 2 Smoke Test ---');

  // 1. Create a dummy institution for testing
  const testOrgName = 'Smoke Test Org ' + Date.now();
  ...
  const fingerprint = crypto.createHash('md5').update(testOrgName + 'Strategic Pivot' + 'http://smoke-test.com').digest('hex');
  const testSignal = {
    ...
    source_tier: 3,
    source_name: 'Smoke Test Scraper',
    source_url: 'http://smoke-test.com',
    impact_direction: 1,
    impact_strength: 8.0, // High strength to trigger 'major' event
    confidence: 1.0,
  ...
  // We call the actual handler logic
  await scoringWorker.handler({ body: JSON.stringify({ secret: process.env.ORION_SECRET }) });
